...
@if(!empty($button['link']) && !empty($button['relative_url']) && !empty($button['secondary_relative_url']))
    <a href="{{ $button['link'] }}" class="content-button block min-w-full relative rounded bg-cover bg-gray-200 my-2" style="padding-top: 36.39%; background-image: url('{{ $button['relative_url'] }}'); ">

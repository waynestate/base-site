        ...
        @foreach($images as $image)
            <div class="w-full relative" aria-labelledby="hero-image-{{ $loop->iteration }}">
                ...
                <div class="md:absolute md:pin-b md:pin-x md:text-white md:text-shadow-dark @if(count($images) > 1) p-6 @else py-4 @if(in_array($page['controller'], config('base.hero_full_controllers')))lg:pb-10 @endif @endif">
                    <div class="row">
                        <div class="mx-4">
                            <h1 id="hero-image-{{ $loop->iteration }}" class="leading-tight text-2xl mb-0 @if(in_array($page['controller'], config('base.hero_full_controllers')))xl:text-5xl @else xl:text-3xl @endif">{{ $image['title'] }}</h1>
                            @if(!empty($image['description']))<div class="pr-2 md:text-lg white-links">{!! $image['description'] !!}</p>@endif
                        </div>
                    </div>
                </div>
            </div>

...
<div class="hero__wrapper w-full relative">
    <div class="hero__primary-image aspect-hero max-h-hero w-full bg-cover bg-center relative{{ $loop->first !== true ? ' lazy' : '' }}" @if($loop->first === true) style="background-image: url('{{ $item['relative_url'] }}')" @else data-src="{{ $item['relative_url'] }}"@endif></div>
    <div class="hero__content-position relative lg:absolute print:relative p-6 lg:p-0 lg:bottom-0 lg:inset-x-0 lg:bg-gradient-darkest lg:pt-20">
        <div class="row">
            <{{ !empty($item['link']) ? 'a href='.$item['link'] : 'div' }} class="hero__content relative block {{ !empty($item['link']) ? 'group no-underline ' : '' }} p-4 lg:pb-2 pl-6 lg:pl-4 pb-8 border-l-12 border-gold border-solid lg:border-0 text-green-900 lg:text-white lg:white-links">

            ...
            @foreach($events as $event)
                <li class="flex items-start break-avoid mb-6">
                    <div class="flex-shrink-0 mt-1 border-2 border-green rounded-sm text-center">
                        <div class="w-12 bg-green text-white leading-none border-b-2 border-green text-sm">{{ apdatetime(date('M' , strtotime($date))) }}</div>
                        <div class="text-green text-2xl leading-tight">{{ apdatetime(date('j' , strtotime($date))) }}</div>
                    </div>

                    <div class="ml-4 flex-grow">
                        <a class="mt-0 block hover:underline" href="{{ $event['url'] }}">
                            {{ $event['title'] }}
                            <span class="visually-hidden"> on {{ apdatetime(date('M d, Y' , strtotime($date))) }}
                                @if(!(bool)$event['is_all_day']) at {{ apdatetime(date('g:i a' , strtotime($event['start_time']))) }}@endif
                            </span>
                        </a>
                        <time class="text-sm text-gray-500" datetime="{{ $date }}T{{ $event['start_time'] }}{{ date('P') }}">

...
@if(!empty($button['link']) && !empty($button['relative_url']))
    <a href="{{ $button['link'] }}" class="block relative rounded bg-green-900 overflow-hidden {{ $class ?? '' }}">
        ...
        <div class="absolute inset-0 p-4 flex items-center">
            <div class="w-full text-lg xl:text-xl font-bold text-white leading-tight text-center">{{ $button['title'] }}</div>
        </div>
    </a>
@endif

...
@foreach($images as $image)
    <div class="min-w-full{{ empty($class) ? ' my-4' : '' }}">
        @if($image['option'] === '')
            @if(!empty($image['excerpt']))
                <a href="{{ $image['link'] }}" class="button expanded text-left">
                    <div class="block text-xl">{{ $image['title'] }}</div>
                    <div class="block pb-1">{{ $image['excerpt'] }}</div>
                </a>
            @else
        ...
        @if($image['option'] === 'Bg gradient green')
            @if(!empty($image['excerpt']))
        ...
        @if($image['option'] === 'Bg image dark')
            <a href="{{ $image['link'] }}" class="block min-w-full relative rounded" style="padding-top: 36.39%; background-image: url('{{ $image['relative_url'] }}'); ">
                <div class="absolute pin p-4 rounded" style="background-color: rgba(5, 33, 30, 0.5);"></div>
        ...
        @if($image['option'] === 'Bg image light')
            <a href="{{ $image['link'] }}" class="block min-w-full relative rounded bg-green-darkest" style="padding-top: 36.39%; background-image: url('{{ $image['relative_url'] }}'); ">
                <div class="absolute pin p-4 rounded" style="background-color: rgba(255, 255, 255, 0.5);"></div>
                <div class="absolute pin p-4 flex @if(!empty($image['excerpt'])) justify-start flex-col @else items-center @endif">
        ...
        @if($image['option'] === 'Icon dark')
            <div class="min-w-full relative rounded bg-gradient-green" style="padding-top: 36.39%;">

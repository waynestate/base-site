...
--}}
<nav id="breadcrumbs-menu" class="breadcrumbs mt-6 mb-2 print:mt-0 max-w-(--breakpoint-3xl) px-4 mx-auto" aria-label="Breadcrumbs">
    <ul class="text-sm @if(!empty($base['flag']))mt:w-4/5 @endif">
        @foreach($breadcrumbs as $key=>$crumb)
            ...
            @elseif($key == (count($breadcrumbs) - 1))
                <li class="font-bold text-green-500 inline">
                    {{ $crumb['display_name'] }}
            @else
                <li class="inline">
                <a href="{{ $crumb['relative_url'] }}" class="text-green-500 hover:underline">{{ $crumb['display_name'] }}</a>
                <span class="icon-right-open px-2"></span>
            @endif
            </li>
        @endforeach

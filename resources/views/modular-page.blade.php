...
@section('content')
    ...
    <div class="grid grid-cols-2 md:grid-cols-2 grid-flow-row-dense items-start gap-y-8 gap-x-4 lg:gap-x-8 mt-8 mb-4">
        @if(!empty($promos))
            @foreach($promos as $group_title => $data)
                @if(!empty($group_title) && !empty($data))
                    @include('components/modular/'.$group_title, ['data' => $data])
                @endif
            @endforeach
        @endif
